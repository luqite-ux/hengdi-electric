...
function mapRow(row: ProductRow): Product {
  ...
  return {
    ...
    category: typeof extra.category === 'string' ? extra.category : original.category,
    subcategory: typeof extra.subcategory === 'string' ? extra.subcategory : original.subcategory,
    gallery: Array.isArray(extra.gallery) ? extra.gallery.filter((x): x is string => typeof x === 'string') : original.gallery,
    sourcePages: Array.isArray(extra.source_pages) ? extra.source_pages.filter((x): x is number => typeof x === 'number') : original.sourcePages,
